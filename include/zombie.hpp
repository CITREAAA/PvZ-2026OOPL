...
public:
    ...
    // 🚩 新增：方便 ScaredyShroom 偵測範圍的行列計算
    // 假設你的地圖格子高度約為 80-100px，寬度約為 80px，請根據你的 GameMap 設定調整除數
    int GetRow() const { return static_cast<int>(m_Transform.translation.y / 80.0f); }
    int GetCol() const { return static_cast<int>(m_Transform.translation.x / 80.0f); }
